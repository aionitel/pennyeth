    ...
    <div>
      ...
      <div>
        <div className='flex-row text-white'>
          <div className='flex text-2xl mt-7 ml-8'>
            <h1 className='mr-2'>Transaction</h1>
            <h1 style={{ display: textOpen ? 'none' : 'block' }}>{tx.hash.slice(0, 15)}</h1>
            <h1 className='hover:cursor-pointer' onClick={() => setTextOpen(true)} style={{ display: textOpen ? 'none' : 'block'}}>...</h1>
            <h1 style={{ display: textOpen ? 'block' : 'none' }}>{tx.hash}</h1>
            <IoMdClose className='mt-1 ml-1 hover:cursor-pointer hover:scale-110 transition-all' onClick={() => setTextOpen(false)} style={{ display: textOpen ? 'block' : 'none'}} />
          </div>
        </div>
      </div>
    </div>
